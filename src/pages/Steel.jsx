...
import { Construction, Collections, ViewStream } from '@mui/icons-material';
...
const Steel = () => {
    ...
    useEffect(() => {
        setProducts(storage.getSteelProducts());
    }, []);
    ...
    // Helper to render a section of products
    const renderProductSection = (sectionTitle, sectionProducts, icon) => {
                ...
                >
                    <Grid container spacing={3.5}>
                        {sectionProducts.map((product) => (
                            <Grid item key={`${sectionTitle}-${product.id}`} xs={12} sm={6} md={4} sx={{ display: 'flex' }}>
    ...
    return (
        <Box sx={{ bgcolor: '#ffffff', minHeight: '100vh' }}>
            {/* 1. Hero Section - Premium Industrial */}
            <Box
                sx={{
                    pt: { xs: 15, md: 22 },
                    pb: { xs: 8, md: 12 },
                    ...
                    >
                        <Box sx={{ textAlign: 'center' }}>
                            ...
                            <Typography variant="h1" fontWeight="900" sx={{ mb: 2, fontSize: { xs: '2.5rem', md: '4.2rem' }, lineHeight: 1.1 }}>
                                Precision <span style={{ color: '#14B8A6' }}>Steel</span> Fabrications
                            </Typography>
                            <Typography variant="h6" sx={{ opacity: 0.8, fontWeight: 400, maxWidth: 700, mx: 'auto', lineHeight: 1.6 }}>
                                Strong, secure, and stylish steel solutions designed to withstand the test of time while elevating architectural aesthetics.
                            </Typography>
                        </Box>
                    </motion.div>
                </Container>
            ...
            <Box sx={{
                ...
                <Container maxWidth="lg">
                    <Box sx={{
                        ...
                        {['All', ...storage.STEEL_CATEGORIES].map((cat) => (
                            <Button
                                key={cat}
            ...
            <Container maxWidth="lg" sx={{ pt: 8, pb: 15 }}>
                <AnimatePresence mode="wait">
                    ...
                    >
                        {selectedCategory === 'All' ? (
                            <>
                                {renderProductSection("Complete Collection", products, <Collections />)}
                                <Divider sx={{ my: 10, borderStyle: 'dashed', opacity: 0.5 }} />
                                {storage.STEEL_CATEGORIES.map(category => (
                ...
                {products.length === 0 && (
                    <Box sx={{ textAlign: 'center', py: 15, bgcolor: '#f8fafc', borderRadius: 8, border: '2px dashed #e2e8f0' }}>
                        <Construction sx={{ fontSize: 60, color: '#94a3b8', mb: 2 }} />
                        <Typography variant="h5" fontWeight="700" color="#475569">
                            Our catalog is currently being updated.
                        </Typography>
                        <Typography variant="body1" color="text.secondary">
                            Please check back soon for our latest steel products.
                        </Typography>
                    </Box>
                )}
            </Container>
