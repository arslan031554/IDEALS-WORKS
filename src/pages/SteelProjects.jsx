...
import { Collections, ViewStream, Engineering } from '@mui/icons-material';
...
const SteelProjects = () => {
    ...
    useEffect(() => {
        setProjects(storage.getSteelProjects());
    }, []);
                    ...
                    >
                        <Box sx={{ textAlign: 'center' }}>
                            ...
                            <Typography variant="h1" fontWeight="900" sx={{ mb: 2, fontSize: { xs: '2.5rem', md: '4.2rem' }, lineHeight: 1.1 }}>
                                Steel <span style={{ color: '#14B8A6' }}>Infrastructure</span>
                            </Typography>
                            <Typography variant="h6" sx={{ opacity: 0.8, fontWeight: 400, maxWidth: 700, mx: 'auto', lineHeight: 1.6 }}>
                                A historical archive of our major steel structures, gates, and industrial fabrication successes.
                            </Typography>
                        </Box>
                    </motion.div>
                </Container>
            ...
            <Box sx={{
                ...
                <Container maxWidth="lg">
                    <Box sx={{
                        ...
                        {['All', ...storage.STEEL_PROJECT_CATEGORIES].map((cat) => (
                            <Button
                                key={cat}
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {selectedCategory === 'All' ? (
                            <>
                                {renderProjectSection("Featured Infrastructure", projects, <Collections />)}
                                <Divider sx={{ my: 10, borderStyle: 'dashed', opacity: 0.5 }} />
                                {storage.STEEL_PROJECT_CATEGORIES.map(category => (
                ...
                {projects.length === 0 && (
                    <Box sx={{ textAlign: 'center', py: 15, bgcolor: '#f8fafc', borderRadius: 8, border: '2px dashed #e2e8f0' }}>
                        ...
                        <Typography variant="body1" color="text.secondary">
                            Steel strength take time to build. Check back soon.
                        </Typography>
                    </Box>
                )}
            </Container>
